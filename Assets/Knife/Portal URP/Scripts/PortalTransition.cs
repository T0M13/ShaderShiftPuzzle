...
namespace Knife.Portal
{
    public class PortalTransition : MonoBehaviour
    {
        ...
        [SerializeField] private GameObject boxVolume;
        [SerializeField] private PortalMesh portalMesh;

        [SerializeField] private List<PortalTransient> transients = new List<PortalTransient>();
        ...
        public Transform EntryPlane { get => entryPlane; set => entryPlane = value; }
        public PortalMesh PortalMesh { get => portalMesh; set => portalMesh = value; }
            ...
            if (IsPortalOpened)
            {
                foreach (var transitingObject in transients)
                {
                    if (transitingObject != null)
                    {
                        ...
                        {
                            float threshold = 0;
                            if (transitingObject.Transient.CanUsePortal)
        ...
        {
            if (!transient.Transient.CanUsePortal) return;
